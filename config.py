...
class Config:
    ...
    # 演示模式：隐藏页面上的教学/展示内容（见 app.css 末尾的 body.demo-mode）。
    #   藏三类：.note 说明块、.teach-block（M.question / M.legacy57 宏）、
    #          .sql-block（M.sql_block，"实际执行的 SQL"深色代码块）。
    # 【为什么做成开关而不是直接删掉那些文案】
    #   那些 .note 块讲的是设计决策，自己开发/复习时要看，演示给面试官时不该出现
    #   —— 页面上顶着"这张表藏着这个项目最关键的设计"，读起来是教程而不是产品。
    #   做成开关，一套代码两种形态，切换只改环境变量。
    #
    # 【SQL 块为什么也归进来（它曾经被排除在外）】
    #   原先的判断是"SQL 让用户知道数字怎么算的，算产品功能"。但标题里那句
    #   "……（手写公式）"暴露了真实受众 —— 农户不关心是不是手写的，这话是说给
    #   懂 SQL 的人听的。所以它和 .note 是同一类东西：展示，不是功能。
    #   将来若想"面数据岗留 SQL、面业务岗全藏"，把 .sql-block 从 app.css 那组
    #   选择器里摘出来、单开一个开关即可。
    #
    # 【⚠️ 这是"视觉隐藏"，不是"访问控制"】
    #   页面由 Jinja 在服务端渲染，CSS 只决定浏览器怎么画 —— 关掉演示模式之后，
    #   文案依然完整地存在于返回的 HTML 里（F12 / 查看源代码可见）。
    #   如果哪天这些内容真成了不该外泄的东西，必须改在模板层用 {% if %} 决定
    #   发不发，CSS 这条路是挡不住的。这里只是演示用，够用。
